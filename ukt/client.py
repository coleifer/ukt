...
class KyotoTycoon(object):
    ...
    def replication(self, sid, ts=None, decode_values=True):
        """
        Replication client.

        :param int sid: replication client server id.
        :param ts: start timestamp for replication (datetime or timestamp). If
            not provided, defaults to current timestamp.
        :param bool decode_values: deserialize values using the configured
            serializer.
        :return: a generator that yields replication events.
        """
        if ts is None:
            ts = time.time()
        ...
        ts = int(ts * 10**9)  # Timestamp expressed in ns.

        with self.ctx() as sock:
            buf = io.BytesIO()
            buf.write(REPLICATION)
            buf.write(b'\x00\x00\x00\x00')  # Flags.
            buf.write(struct.pack('>QH', ts, sid))
            sock.send(buf.getvalue())

            self.check_error(sock, REPLICATION)

            while True:
                magic = sock.recv(1)
                ts, = struct_q.unpack(sock.recv(8))
                if magic == SYNC:
                ...
                log_size, = struct_i.unpack(sock.recv(4))
                log_data = sock.recv(log_size)
                yield self._parse_ulog(memoryview(log_data), decode_values)

    def _parse_ulog(self, data, decode_values):
        ...
        if op == REPL_REMOVE:
            ...
            raise KTError('unsupported replication operation: %s' % hex(op))
        return log
